...
class RaspberryPiService {
    ...
    startKiosk() {
        ...
        return new Promise((resolved) => {
            const command = `/usr/bin/chromium-browser --enable-native-gpu-memory-buffers -start-maximized --no-sandbox --kiosk --disable-infobars  http://127.0.0.1:${env_1.env.serverPort}`;
            child_process_1.exec(command, (err, stdout, stderr) => {
                if (err) {
                    console.log(err);
                }
